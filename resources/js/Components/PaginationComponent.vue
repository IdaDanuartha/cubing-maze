<template>
    <nav>
        <ul :class="`flex justify-between mb-0 mt-4`">
        <p class="text-gray-500 dark:text-gray-400 text-sm">Results: {{ data.from }} - {{ data.to }} of {{ data.total }}</p>
        <div class="flex">
            <li class="nav-item"
...
.page-item .page-link,
.nav-item .nav-link {
    @apply transition duration-300 px-2 py-[2px] mx-0.5;
}
...
html.dark .page-item .page-link:hover {
    @apply bg-fourth-color/30 rounded text-gray-100; 
}

html.dark .page-item.active .page-link {
    @apply bg-fourth-color/30 rounded border-0 text-gray-100;
}
</style>